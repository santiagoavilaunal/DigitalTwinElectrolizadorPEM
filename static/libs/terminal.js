...
var Terminal={
	...
	TerminalConstructor : function (containerId) {
		...
		this.print = function (message) {
			...
			newLine.innerHTML = this.ansiToHtml(message);
		...
		this.ansiToHtml= function(ansiString) {
			const ansiStyles = {
				'30': 'color: black;',
				'31': 'color: red;',
				'32': 'color: green;',
			...
			const extendedColors = (code) => {
				const colorCode = parseInt(code, 10);
				if (colorCode >= 0 && colorCode <= 255) {
					return `color: ${ansi256ToRgb(colorCode)};`;
				}
				return '';
			};
		
			const ansi256ToRgb = (code) => {
				const r = Math.floor((code / 36) % 6) * 51;
				const g = Math.floor((code / 6) % 6) * 51;
				const b = Math.floor(code % 6) * 51;
				return `rgb(${r}, ${g}, ${b})`;
			};
		
			return ansiString.replace(/\x1b\[([0-9;]+)m/g, (match, p1) => {
				const codes = p1.split(';');
				let htmlStyle = '';
		
				codes.forEach((code, index) => {
					if (code === '38' && codes[index + 1] === '5') {
						htmlStyle += extendedColors(codes[index + 2]);
					} else if (ansiStyles[code]) {
						htmlStyle += ansiStyles[code];
					} else if (code === '0') {
						htmlStyle = ''; // Reset style
					}
				});
		
				return htmlStyle ? `<span style="${htmlStyle}">` : '</span>';
			});
		}
	}
}
